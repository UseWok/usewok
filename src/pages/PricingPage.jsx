...
export default function PricingPage() {
  ...
  return (
    <div className="min-h-screen bg-background font-be py-12">
      <div className="max-w-3xl mx-auto px-6">
        {isAdmin && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="mb-8 p-4 bg-primary/5 border border-primary/20 rounded-xl">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">Vous bénéficiez de <strong>1 000 000 crédits</strong> automatiquement.</p>
          </motion.div>
        )}
          ...
          {plans.map((plan, idx) => {
            ...
            return (
              <motion.div
                ...
                className={`relative bg-card border rounded-2xl p-5 flex flex-col ${plan.popular ? 'border-primary shadow-lg shadow-primary/10' : 'border-border'}`}
              >
                {plan.popular && (
