...
const ContactModal = ({ onClose }) => {
  ...
  const inp = { width: '100%', background: '#1A1A1A', border: '1px solid #2A2A2A', borderRadius: 8, padding: '10px 12px', fontSize: 13, color: '#fff', outline: 'none', boxSizing: 'border-box', fontFamily: F };
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
      ...
      <div style={{ background: '#111', border: '1px solid #2A2A2A', borderRadius: 16, padding: 32, width: '100%', maxWidth: 460, position: 'relative', fontFamily: F }}>
        <button onClick={onClose} style={{ position: 'absolute', top: 14, right: 14, background: 'none', border: 'none', cursor: 'pointer', color: '#555', padding: 4 }}><X size={15} /></button>
        {submitted ? (
          <div style={{ textAlign: 'center', padding: '2rem 0' }}>
            ...
            <button onClick={onClose} style={{ marginTop: 24, padding: '11px 32px', background: '#fff', color: '#111', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>Got it</button>
          </div>
        ) : (
          <>
            <h2 style={{ fontSize: 20, fontWeight: 700, color: '#fff', margin: '0 0 6px' }}>Contact us</h2>
            <p style={{ fontSize: 13, color: '#888', margin: '0 0 20px' }}>Our team will reply within 24h.</p>
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {[['first_name','First name'],['last_name','Last name']].map(([k,l]) => (
                  <div key={k}><label style={{ display: 'block', fontSize: 11, color: '#666', marginBottom: 5 }}>{l}</label><input required value={form[k]} onChange={set(k)} placeholder={l} style={inp} /></div>
                ))}
              </div>
              {[['email','Professional email','email'],['website','Website','text'],['role','Your role','text']].map(([k,l,t]) => (
                <div key={k}><label style={{ display: 'block', fontSize: 11, color: '#666', marginBottom: 5 }}>{l}</label><input required type={t} value={form[k]} onChange={set(k)} placeholder={l} style={inp} /></div>
              ))}
              <div><label style={{ display: 'block', fontSize: 11, color: '#666', marginBottom: 5 }}>Message</label><textarea required value={form.message} onChange={set('message')} rows={3} style={{ ...inp, resize: 'none' }} /></div>
              <button type="submit" style={{ padding: '11px 0', background: '#fff', color: '#111', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer', marginTop: 4 }}>Send</button>
            </form>
          </>
        )}
      </div>
...
function Toggle({ on, onToggle }) {
  return (
    <button onClick={onToggle} style={{
      ...
      background: on ? '#5B5FEF' : 'rgba(255,255,255,0.15)',
...
function Cell({ value }) {
  ...
  if (value === true || value === 'Yes') return <Check style={{ width: 14, height: 14, color: T2 }} />;
...
export default function PricingPage() {
  ...
  const PlanBtn = ({ plan }) => {
    ...
    if (current) return (
      <button style={{ width: '100%', padding: '8px 0', borderRadius: 8, fontSize: 13, fontWeight: 500, fontFamily: F, background: 'rgba(255,255,255,0.08)', color: T2, border: '1px solid rgba(255,255,255,0.12)', cursor: 'default' }}>
        Current plan
      </button>
    );
    if (isFree) return (
      <button onClick={() => navigate('/app')} style={{ width: '100%', padding: '8px 0', borderRadius: 8, fontSize: 13, fontWeight: 500, fontFamily: F, background: 'rgba(255,255,255,0.1)', color: T1, border: '1px solid rgba(255,255,255,0.14)', cursor: 'pointer', transition: 'background 150ms' }}
    ...
    return (
      <button onClick={() => handleUpgrade(plan)} style={{ width: '100%', padding: '8px 0', borderRadius: 8, fontSize: 13, fontWeight: 500, fontFamily: F, background: 'rgba(255,255,255,0.1)', color: T1, border: '1px solid rgba(255,255,255,0.14)', cursor: 'pointer', transition: 'background 150ms' }}
        onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.16)'}
        onMouseLeave={e => e.currentTarget.style.background = 'rgba(255,255,255,0.1)'}>
        Get started
      </button>
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG, fontFamily: F, color: T1, overflowX: 'hidden' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '64px 32px 120px' }}>
        ...
        <div style={{ marginTop: 80, padding: '36px 40px', border: `1px solid ${BORDER}`, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 20 }}>
          <div>
            <div style={{ fontSize: 18, fontWeight: 700, color: T1, marginBottom: 6 }}>Need a custom plan?</div>
            <div style={{ fontSize: 14, color: T3 }}>For large teams, custom contracts or invoicing — contact us.</div>
          </div>
          <button onClick={() => setShowContact(true)} style={{ padding: '10px 24px', borderRadius: 8, border: `1px solid rgba(255,255,255,0.18)`, background: 'transparent', color: T1, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: F, transition: 'background 150ms' }}
