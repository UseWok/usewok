        ...
        <nav className="flex-1 overflow-y-auto overflow-x-hidden px-2 py-2 flex flex-col gap-0.5">
          ...
          <div className="mt-3">
              ...
              transition={{ duration: 0.2 }} className="overflow-hidden">
                    ...
                    let unlocked = false;
                    if (userPlan) {
                      if (f.planId === 'expert') unlocked = userPlan.ultimate_access;else
                      if (f.planId === 'advanced') unlocked = userPlan.internet_access || f.label === 'Unlimited Discussions' && userPlan.max_discussions === 0;else
                      if (f.planId === 'essential') unlocked = userPlan.file_upload;
                    }
                    return (
        ...
        <div className="flex-shrink-0 px-2 pb-4 flex flex-col items-center gap-1">
          ...
          <div className="w-full flex items-center gap-3 px-3 py-2">
            ...
            className="relative w-7 h-7 flex items-center justify-center flex-shrink-0 rounded-md hover:bg-black/5 transition-colors">
              ...
              className="absolute top-0.5 right-0.5 w-2 h-2 bg-coral rounded-full border-[1.5px] border-white hidden" />
              }
            </button>
          </div>
        </div>
