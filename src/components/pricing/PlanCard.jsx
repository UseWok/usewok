...
export default function PlanCard({ plan, billing, isCurrent, onCta, loading, ctaLabel }) {
  ...
  const leftSubtle = isReco ? 'rgba(250,249,246,0.55)' : 'rgba(21,19,15,0.5)';
  ...
  return (
    <div className="uw-pcard" style={{
      ...
      <div className="uw-pcard-body" style={{ display: 'flex', flex: 1 }}>
        {/* ── LEFT ── */}
        <div className="uw-pcard-left" style={{
          ...
          {billing === 'yearly' && !isFree && plan.price_yearly && (
            <div style={{ fontSize: 11, color: isReco ? 'rgba(250,249,246,0.4)' : 'rgba(21,19,15,0.4)' }}>
              soit {plan.price_yearly}€ facturé annuellement
            </div>
          )}
          <p style={{ fontSize: 12.5, color: isReco ? 'rgba(250,249,246,0.55)' : 'rgba(21,19,15,0.5)', lineHeight: 1.6, marginTop: 'auto' }}>
            {plan.description || ''}
          </p>
        </div>
        ...
        {/* ── RIGHT ── */}
        <div style={{
          background: CREAM, padding: '26px 24px', flex: 1,
          display: 'flex', flexDirection: 'column',
        }}>
      ...
      <style>{`
        @media (max-width: 900px) {
          .uw-pcard-body { flex-direction: column !important; }
