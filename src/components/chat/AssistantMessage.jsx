...
export default function AssistantMessage({ content, isGenerating, query }) {
  ...
  // Dynamic Intelligence Rendering (Hardcoded in English)
  useEffect(() => {
    ...
    timers.push(setTimeout(() => {
      ...
      // Trigger fade out of the visualizer after completion
      setTimeout(() => setIsFullyComplete(true), 800);
    }, 3200));

    return () => timers.forEach(clearTimeout);
  }, [isGenerating, query]);

  if (isGenerating && !isFullyComplete) {
    return (
  ...
  // Once generated or fully complete, show the actual text payload
  if (!isGenerating || isFullyComplete) {
