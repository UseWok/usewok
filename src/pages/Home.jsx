...
async function runScan(inputUrl, userId, features) {
  ...
  if (existing) await base44.entities.BusinessProfile.update(existing.id, fields);else
  ...
  const handleDeleteDomain = async (p) => {
    try {
      ...
      if (activeUrl === p.site_url) {
        ...
        if (rem.length > 0) switchDomain(rem[0]);else
        {setActiveUrl(null);setActiveDomain(null);}
      }
    } catch {}
  };
