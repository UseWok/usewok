...
const WOK_SYSTEM = `You are a brilliant, candid financial expert. You speak like a smart friend — direct, warm, and actionable.
...
export default function ChatPage() {
  ...
  const [input, setInput] = useState(() => {
    const saved = localStorage.getItem('wok_saved_input');
    if (saved) { localStorage.removeItem('wok_saved_input'); return saved; }
  ...
  const getDailyUsed = () => { try { return JSON.parse(localStorage.getItem('wok_daily_usage') || '{}')[todayKey] || 0; } catch { return 0; } };
  const incrementDailyUsed = () => { try { const d = JSON.parse(localStorage.getItem('wok_daily_usage') || '{}'); d[todayKey] = (d[todayKey] || 0) + 1; localStorage.setItem('wok_daily_usage', JSON.stringify(d)); } catch { } };
      ...
      {isSidebarOpen && (
        <aside className="w-[260px] flex-shrink-0 h-full bg-white border-r border-[#E5E5E5] flex flex-col z-40">
          ...
          <div className="px-3 space-y-0.5 mt-2">
            {navItems.map((item) => (
              <button key={item.label} onClick={() => navigate(item.path)} className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-[13px] font-medium text-gray-600 hover:bg-gray-100 transition-colors">
          ...
          <div className="p-3 border-t border-[#E5E5E5] relative" ref={profileMenuRef}>
            <AnimatePresence>
              {isProfileMenuOpen && (
                <motion.div 
                  initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 5 }} transition={{ duration: 0.1 }}
                  className="absolute bottom-[calc(100%+8px)] left-3 w-[240px] bg-white border border-[#E5E5E5] rounded-xl shadow-lg py-1.5 z-50 p-1"
                >
                  <div className="px-3 py-2 border-b border-[#E5E5E5] mb-1">
                    ...
                    <p className="text-[12px] text-gray-500 truncate">Plan: {userPlan?.name || 'Free Plan'}</p>
                  </div>
                  
                  {/* MODIFIED: Trigger IframeModal for Settings */}
                  <button onClick={() => { setIsProfileMenuOpen(false); setIframeModal({open:true, url:'/settings'}); }} className="w-full text-left px-3 py-2 text-[13px] text-gray-700 hover:bg-gray-100 flex items-center gap-2.5 transition-colors rounded-md">
                    <Settings className="w-4 h-4 text-gray-500" /> Settings
                  </button>
                  
                  {/* MODIFIED: Changed "Support tickets" to "Support" and trigger IframeModal */}
                  <button onClick={() => { setIsProfileMenuOpen(false); setIframeModal({open:true, url:'/support'}); }} className="w-full text-left px-3 py-2 text-[13px] text-gray-700 hover:bg-gray-100 flex items-center gap-2.5 transition-colors rounded-md">
                    <LifeBuoy className="w-4 h-4 text-gray-500" /> Support
                  </button>
                  ...
                  {/* Keeping "I have a code" as it triggers its own behavior/modal outside of iframe context if preferred, or leave as placeholder */}
                  <button onClick={() => { setIsProfileMenuOpen(false); }} className="w-full text-left px-3 py-2 text-[13px] text-gray-700 hover:bg-gray-100 flex items-center gap-2.5 transition-colors rounded-md">
        ...
        <div className={`flex flex-1 rounded-[16px] overflow-hidden bg-white shadow-sm border border-[#E5E5E5] transition-all duration-300 ${hasStarted ? 'flex-row' : 'flex-col max-w-3xl mx-auto w-full'}`}>
          ...
          <div className={`flex flex-col bg-white overflow-hidden ${hasStarted ? 'w-[450px] border-r border-[#E5E5E5] z-10' : 'w-full h-full'}`}>
            ...
            <div ref={scrollContainerRef} className={`flex-1 overflow-y-auto px-5 py-6 space-y-4 pb-4 [&::-webkit-scrollbar]:hidden ${!hasStarted ? 'flex flex-col justify-end' : ''}`}>
              {!hasStarted && (
                 <div className="flex flex-col items-center justify-center text-center opacity-30 w-full mb-10">
                   <img src={LOGO_URL} alt="Wok" className="w-10 h-10 object-contain mb-3" />
            ...
            <div className={`flex-shrink-0 flex flex-col p-4 pt-2 bg-white ${!hasStarted ? 'pb-10' : ''}`}>
              <ChatInputBar input={input} setInput={setInput} onSend={sendMessage} onStop={handleStop} isLoading={isLoading} />
