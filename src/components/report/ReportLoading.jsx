  ...
  return (
    <div style={{ minHeight: '100vh', background: SURFACE, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: F }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 13, color: INK3, margin: 0 }}>Loading…</p>
      </div>
...
export function ScanInProgress() {
  return (
    <div style={{ minHeight: '100vh', background: CARD_DARK, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 32, fontFamily: F }}>
      ...
      <div style={{ fontSize: 19, fontWeight: 700, color: '#FFFFFF', marginBottom: 5 }}>Analysis in progress…</div>
      <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)' }}>8 AI engines · Results in ~60 seconds</div>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
    </div>
  );
}
...
export function ReportEmpty({ onBack }) {
  return (
    <div style={{ minHeight: '100vh', background: SURFACE, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 14, padding: 24, textAlign: 'center', fontFamily: F }}>
      ...
      <p style={{ fontSize: 17, fontWeight: 800, color: '#1A1814', margin: 0 }}>No analysis available</p>
      <p style={{ fontSize: 13, color: INK3, margin: 0, maxWidth: 260 }}>Run an analysis from the home page to see your report.</p>
      <button onClick={onBack} style={{ padding: '11px 22px', background: '#15130F', color: '#fff', border: 'none', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: F }}>← Back</button>
    </div>
  );
}