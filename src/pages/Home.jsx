...
const LogoAuto = () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke={INK} strokeWidth="1.5" /><path d="M8 12h8M12 8l4 4-4 4" stroke={INK} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>;
...
function EnginesDropdown({ selected, onToggle, onClose }) {
  ...
  return (
    <div ref={ref} style={{
      ...
      <div onClick={() => onToggle('auto')}
        onMouseEnter={(e) => { if (!isAutoSelected) e.currentTarget.style.background = '#FAF6EF'; }}
        onMouseLeave={(e) => { e.currentTarget.style.background = WHITE; }}
        style={{ display: 'flex', alignItems: 'flex-start', gap: 11, padding: '9px 14px 10px', cursor: 'pointer', background: WHITE, borderBottom: `1px solid ${BORDER}`, transition: 'background 100ms' }}>
      ...
      {otherEngines.map((e) => {
        ...
        return (
          <div key={e.id} onClick={() => onToggle(e.id)}
            style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '6px 14px', cursor: 'pointer', background: 'transparent', transition: 'background 100ms' }}
  ...
  return (
    <div style={{ position: 'relative' }}>
      <button onClick={startListening}
        style={{ width: 32, height: 32, border: 'none', background: listening ? `rgba(241,123,93,0.12)` : 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 7, transition: 'background 120ms', flexShrink: 0, outline: 'none' }}
...
function SubmitButton({ onClick, loading }) {
  ...
  return (
    <div style={{ position: 'relative', flexShrink: 0 }}>
      ...
      {hovered && (
        <div style={{ position: 'absolute', bottom: 'calc(100% + 6px)', left: '50%', transform: 'translateX(-50%)', background: INK, color: WHITE, fontSize: 11, fontWeight: 600, borderRadius: 6, padding: '4px 9px', whiteSpace: 'nowrap', pointerEvents: 'none', zIndex: 100, letterSpacing: '0.01em' }}>
          Analyser
...
async function extractUrlFromText(text) {
  ...
  try {
    const res = await base44.integrations.Core.InvokeLLM({
      ...
      response_json_schema: { type: 'object', properties: { url: { type: 'string' }, name: { type: 'string' } } }
    });
    const extracted = res?.url || null;
    const name = res?.name || null;
    if (!extracted || extracted === 'null' || !extracted.includes('.')) { LLM_CACHE[key] = null; return null; }
  ...
  return (
    <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
      <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
        ...
        <circle cx={size / 2} cy={size / 2} r={R} fill="none" stroke={CORAL} strokeWidth={sw} strokeDasharray={circ} strokeDashoffset={circ * (1 - score / 100)} strokeLinecap="round" />
      </svg>
...
export default function Home() {
  ...
  const handleSubmitSearch = async () => {
    ...
    if (direct) { startScan(direct); setSearchQuery(''); return; }
    setExtracting(true);
    setTrollError(false);
    ...
    <div style={{ minHeight: '100vh', background: BG, fontFamily: F }}>
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 24px 80px' }}>
        ...
        <div style={{ position: 'relative', marginBottom: trollError ? 8 : 16 }}>
          <div style={{ background: WHITE, border: `1px solid ${trollError ? '#EF4444' : BORDER}`, borderRadius: 10, padding: '9px 9px 9px 14px', display: 'flex', alignItems: 'center', gap: 8, transition: 'border-color 200ms' }}>
            ...
            <textarea
              ...
              rows={1}
              style={{ flex: 1, border: 'none', outline: 'none', background: 'transparent', fontSize: 13.5, color: INK, fontFamily: F, minWidth: 0, resize: 'none', lineHeight: '1.5', minHeight: '20px', maxHeight: `${4 * 1.5 * 13.5}px`, overflowY: 'auto' }}
              onInput={(e) => {
          ...
          <AnimatePresence>
            {showEngines && (
              <EnginesDropdown selected={selectedEngines} onToggle={toggleEngine} onClose={() => setShowEngines(false)} />
            )}
          </AnimatePresence>
        </div>
          ...
          {isScanningActive ? (
            ...
            <div style={{ background: CARD_BG, borderRadius: 14, padding: '16px 18px', cursor: 'pointer', position: 'relative' }}
              ...
              <div style={{ position: 'absolute', top: 13, right: 14 }} onClick={(e) => e.stopPropagation()}>
                <ScanStatusIndicator lastScan={activeProfile?.last_scan} planId={user?.subscription_plan || 'free'} onScan={() => startScan(activeProfile.site_url)} scanning={isScanningActive} />
              </div>
