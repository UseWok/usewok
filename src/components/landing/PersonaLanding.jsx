...
/**
 * PersonaLanding — landing page réutilisable pour la stratégie "Pages Personas".
 * Design 100% identique à la landing principale. Seul le contenu textuel varie
 * via la prop `content` (H1, intro, cas d'usage, FAQ, etc.) — ~30% de texte unique
 * par page pour rester conforme aux règles de contenu dupliqué de Google.
 */
export default function PersonaLanding({ content }) {
    ...
    <div className="uw-landing">
      ...
      <section className="hero">
        <div className="wrap hero-inner">
          <span className="eyebrow"><span className="dot"></span>{content.hero.eyebrow}</span>
          <h1>{content.hero.title}</h1>
          <p>{content.hero.subtitle}</p>
          <div className="hero-ctas">
            <button className="btn btn-dark" onClick={() => setShowAnalyze(true)}>{content.hero.ctaPrimary}</button>
            <button className="btn btn-outline" onClick={() => scrollTo(howRef)}>{content.hero.ctaSecondary}</button>
          </div>
      ...
      <div className="trustbar">
        <div className="wrap">
          <span className="eyebrow" style={{justifyContent:'center', display:'flex'}}>{content.trustLabel}</span>
        ...
        <div className="wrap">
          <div className="panels">
            <div className="panel panel-light">
              <div className="k">{content.understand.eyebrow}</div>
              <h3>{content.understand.title}</h3>
              <div className="feat">
                <b>{content.understand.feat1Title}</b>
                <p>{content.understand.feat1Text}</p>
              </div>
              <div className="feat">
                <b>{content.understand.feat2Title}</b>
                <p>{content.understand.feat2Text}</p>
              </div>
            </div>
            <div className="panel panel-glow">
              <div className="prompt-card">
                <p>{content.promptCard}<span className="caret"></span></p>
          ...
          <div className="panels2">
            ...
            <div className="panel-check">
              <h3>{content.reliability.title}</h3>
              <p>{content.reliability.subtitle}</p>
              {content.reliability.items.map((it, i) => (
      ...
      <section ref={howRef}>
        <div className="wrap">
          <div className="explain-head">
            ...
            <p style={{fontSize:15, color:'var(--ink-faint)', marginTop:14, lineHeight:1.6}}>{content.how.intro}</p>
          </div>
          <div className="explain-grid">
            <div className="explain-col">
              <div className="explain-item">
                <b>{content.how.items[0].title}</b>
                <p>{content.how.items[0].text}</p>
              </div>
              <div className="explain-item">
                <b>{content.how.items[1].title}</b>
                <p>{content.how.items[1].text}</p>
              </div>
            </div>
            ...
            <div className="explain-col right">
              <div className="explain-item right">
                <b>{content.how.items[2].title}</b>
                <p>{content.how.items[2].text}</p>
              </div>
              <div className="explain-item right">
                <b>{content.how.items[3].title}</b>
                <p>{content.how.items[3].text}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section>
        <div className="wrap">
          <div className="confidence-top">
            <h2>{content.confidence.title}</h2>
            <div>
              <p>{content.confidence.subtitle}</p>
              <button className="btn btn-dark" onClick={goRegister}>See features</button>
            </div>
          </div>
          <div className="cards3">
            {content.confidence.cards.map((c, i) => (
              <div className="card3" key={i}><h4>{c.title}</h4><p>{c.text}</p></div>
            ))}
          </div>
        </div>
      </section>
          ...
          <div className="test-grid">
            {content.testimonials.map((t, i) => (
              <div className="test-card" key={i}>
                ...
                <p>{t.text}</p>
                <div className="test-who"><div className="av"></div><div><b>{t.name}</b><span>{t.role}</span></div></div>
              </div>
            ))}
          </div>
          <div className="stat-grid">
            <div className="stat-card stat-1"><span className="big">{content.stats[0].big}</span><span className="lbl">{content.stats[0].lbl}</span></div>
            <div className="stat-card stat-2"><span className="big">{content.stats[1].big}</span><span className="lbl">{content.stats[1].lbl}</span></div>
            <div className="stat-card stat-3"><span className="big">{content.stats[2].big}</span><span className="lbl">{content.stats[2].lbl}</span></div>
          </div>
        </div>
      </section>
      ...
      <section>
        <div className="wrap">
          <div className="explain-head">
            ...
            <p style={{fontSize:15, color:'var(--ink-faint)', marginTop:14, lineHeight:1.6}}>{content.pricingIntro}</p>
          </div>
          <div style={{maxWidth:440, margin:'0 auto'}}>
            <div className="card3" style={{background:'linear-gradient(165deg, var(--orange-pale) 0%, var(--cream-2) 100%)', border:'1px solid var(--orange)', minHeight:'auto'}}>
              <div>
                ...
                <p style={{marginTop:14}}>{content.pricingCard}</p>
              </div>
        ...
        <div className="wrap">
          <div className="finalcta">
            <div className="cta-card">
              <h3 className="serif">{content.finalCta}</h3>
              <button className="btn btn-dark" onClick={() => setShowAnalyze(true)}>Start analyzing →</button>
            </div>
          </div>
        </div>
