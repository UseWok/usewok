    ...
    <html>
      ...
      <body>
        ...
        <script type="text/babel" data-type="module" data-presets="react">
          ...
          ${compiledCode.js.replace(/`/g, '\\`').replace(/\$\{/g, '\\${')}
          
          class ErrorBoundary extends React.Component {
            constructor(props) { super(props); this.state = { hasError: false }; }
            static getDerivedStateFromError() { return { hasError: true }; }
            componentDidCatch(error) {
              try { window.parent.postMessage({ type: 'WOK_RUNTIME_ERROR', message: error.message || error.toString() }, '*'); } catch(e) {}
            }
            render() {
          ...
          try {
            if (typeof App !== 'undefined') {
              ...
              try { window.parent.postMessage({ type: 'WOK_RUNTIME_SUCCESS' }, '*'); } catch(e) {}
            } else {
              throw new Error("Component must be named 'App'.");
            }
          } catch(err) {
            try { window.parent.postMessage({ type: 'WOK_RUNTIME_ERROR', message: err.message }, '*'); } catch(e) {}
          }
        </script>
      </body>
    </html>
