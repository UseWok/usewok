...
import { Settings, Sparkles, Binary, FileUp, Zap, Target, LineChart } from 'lucide-react';

export default function ChatInputBar({ input, setInput, onSend, isLoading, onOpenIframe, setFiles, canUploadFiles }) {
  const [showAIConfig, setShowAIConfig] = useState(false);
  const [apexEquityMode, setApexEquityMode] = useState(false);
  
  // Radio button logic: only one choice
  ...
  return (
    <div className="flex flex-col gap-2 font-sans relative" ref={configRef}>
      ...
      {/* AI LEARNING STRATEGIES POPOVER */}
      {showAIConfig && (
        <div className="absolute bottom-[calc(100%+12px)] left-0 w-[340px] bg-white border border-[#E5E5E5] rounded-xl shadow-[0_12px_40px_rgba(0,0,0,0.12)] z-50 p-2 font-sans">
