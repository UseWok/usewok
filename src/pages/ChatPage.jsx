...
export default function ChatPage() {
  ...
  }, [messages, isLoading, discussMode, currentWorkspace]);
    ...
    <div className="flex font-sans h-screen w-full bg-white overflow-hidden antialiased">
        ...
        <div className="w-[260px] flex flex-col h-full">
          ...
          <div className="p-4 border-b border-black/5 relative" ref={workspaceRef}>
            ...
            {showWorkspaceSwitcher && (
              <div className="absolute top-[calc(100%+0px)] left-4 right-4 bg-white border border-[#E5E5E5] rounded-md shadow-xl py-2 z-50 p-1.5 transition-none">
                {workspaces.map(w => (
          ...
          <div className="px-4 py-3 border-b border-black/5"><button onClick={() => navigate('/')} className="flex items-center justify-center gap-2 w-full py-2.5 bg-[#0080ff] text-white rounded-md text-[13px] font-bold hover:bg-[#0066cc] shadow-sm transition-none"><Plus className="w-4 h-4" /> New chat</button></div>

          <div className="px-4 space-y-0.5 mt-3">
            {navItems.map((item) => (
              <button key={item.label} onClick={() => navigate(item.path)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-md text-[13px] font-medium transition-none ${item.active ? 'bg-white shadow-sm border border-[#E5E5E5] text-gray-900 font-bold' : 'text-gray-600 hover:bg-white/50 border border-transparent'}`}>
          ...
          <div className="flex-1 overflow-y-auto px-4 mt-6">
             ...
             <ul className="space-y-0.5">
                {discussions?.map((d, idx) => (
                  <li key={d.id} draggable onDragStart={() => setDraggedItemIdx(idx)} onDragOver={(e) => { e.preventDefault(); setDragOverIdx(idx); }} onDrop={() => handleDrop(idx)} onClick={() => navigate(`/chat?conversationId=${d.id}`)} className={`relative flex items-center justify-between px-3 py-2.5 rounded-md cursor-pointer group transition-none ${conversationId === d.id ? 'bg-white shadow-sm border border-[#E5E5E5]' : 'border border-transparent hover:bg-white/50'}`}>
                    {editingId === d.id ? (
          ...
          <div className="p-4 border-t border-black/5 relative" ref={profileMenuRef}>
            {isProfileMenuOpen && (
              <div className="absolute bottom-[calc(100%+12px)] left-4 w-[240px] bg-white border border-[#E5E5E5] rounded-md shadow-[0_8px_30px_rgba(0,0,0,0.08)] py-1.5 z-50 font-sans p-1.5 transition-none">
                ...
                <button onClick={() => { setIsProfileMenuOpen(false); setShowSettingsModal(true); }} className="w-full text-left px-3 py-2 text-[13px] text-[#707070] hover:bg-gray-50 flex items-center gap-2.5 rounded-md transition-none"><Settings className="w-4 h-4 text-gray-400" /> Settings</button>
                <button onClick={() => { setIsProfileMenuOpen(false); setShowSupportModal(true); }} className="w-full text-left px-3 py-2 text-[13px] text-[#707070] hover:bg-gray-50 flex items-center gap-2.5 rounded-md transition-none"><LifeBuoy className="w-4 h-4 text-gray-400" /> Support</button>
      ...
      <div className="flex-1 flex overflow-hidden bg-white relative">
        ...
        <div className="flex flex-1 overflow-hidden w-full h-full">
          ...
          {/* CHAT COLUMN (Strict 23% approximation via min/max) */}
          <div className={`flex flex-col bg-white overflow-hidden transition-none ${hasStarted ? 'w-[23%] min-w-[300px] max-w-[340px] border-r border-[#E5E5E5] z-10' : 'w-full h-full justify-center max-w-3xl mx-auto'}`}>
          ...
          {hasStarted && (
            <div className="flex-1 bg-white p-3 overflow-hidden flex flex-col w-[77%]">
