...
export function StatusBadge({ status }) {
  const map = {
    done: { label: 'Terminé', bg: '#10B981', color: '#fff' },
    running: { label: 'En cours', bg: 'rgba(249,115,22,0.14)', color: '#F97316' },
    failed: { label: 'Échec', bg: 'rgba(239,68,68,0.12)', color: '#EF4444' },
  };
...
export const AGENT_LABELS = { crawl: 'Crawl', freshness: 'Fraîcheur', seo: 'SEO Structurel', content: 'Qualité Contenu' };
...
export function AgentDots({ agents }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
      {AGENT_ORDER.map(k => (
        <div key={k} title={`${AGENT_LABELS[k]} : ${a[k] === 'done' ? 'Terminé' : 'En attente'}`}
...
export function AgentChip({ agentKey, status }) {
  ...
  return (
    <span style={{
      ...
      {AGENT_LABELS[agentKey]}: {done ? 'Terminé' : 'En attente'}
    </span>
  );
}