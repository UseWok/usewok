...
import Toggle from '@/components/ui/Toggle'; // Assuming shared Toggle exists
import { Settings, Sparkles, Binary, FileUp, Zap, Target, LineChart, Briefcase, ChevronDown, Plus, Check } from 'lucide-react';

export default function ChatInputBar({ input, setInput, onSend, isLoading, currentWorkspace, workspaces, setWorkspaces, showWorkspaceSwitcher, setShowWorkspaceSwitcher, workspaceRef, handleCreateWorkspace, handleSwitchWorkspace }) {
  ...
  return (
    <div className="flex flex-col gap-2 font-sans relative">
      ...
      {/* WORKSPACE SWITCHER INTEGRATED IN CHAT AREA (Image 3 reference) */}
      <div className="self-start relative" ref={workspaceRef}>
         <button 
            onClick={() => setShowWorkspaceSwitcher(!showWorkspaceSwitcher)}
            className="flex items-center gap-2 px-3 py-1.5 bg-white border border-[#E5E5E5] rounded-lg hover:bg-gray-50 transition-colors shadow-sm text-gray-700"
          >
          ...
          {showWorkspaceSwitcher && (
            <div className="absolute bottom-[calc(100%+8px)] left-0 w-[240px] bg-white border border-black rounded-lg shadow-xl py-1.5 z-50 p-1">
              {workspaces.map(w => (
              ...
              <button onClick={handleCreateWorkspace} className="w-full text-left px-3 py-2 text-[13px] font-bold text-[#0080ff] hover:bg-gray-50 flex items-center gap-2 transition-colors rounded-md">
      ...
      <div className="relative" ref={configRef}>
        {/* AI LEARNING STRATEGIES POPOVER (No animations, black border) */}
        {showAIConfig && (
          <div className="absolute bottom-[calc(100%+12px)] left-0 w-[340px] bg-white border border-black rounded-xl shadow-2xl z-50 p-2 font-sans">
            ...
            <div className="pt-4 pb-1 space-y-1">
              ...
              {learningStrategies.map((strategy, idx) => (
                <div key={strategy.id} className="relative">
                  ...
                  {/* Elegant non-touching separating line */}
                  {idx < learningStrategies.length - 1 && (
                    <div className="absolute left-[44px] right-4 bottom-0 h-px bg-[#E5E5E5]"></div>
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="bg-white border border-[#E5E5E5] rounded-[16px] p-2 flex items-center gap-2 shadow-sm relative z-10 transition-shadow hover:shadow-md focus-within:shadow-md">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <button 
              ...
              className="p-2 bg-[#0080ff] text-white rounded-xl hover:bg-[#0066cc] transition-colors disabled:opacity-40 disabled:cursor-not-allowed shadow-sm"
            >
