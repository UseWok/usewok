...
// ── Analytics are PER BUILD — keyed by convId ─────────────────
function getAnalyticsKey(convId) {
  return convId ? `wok_analytics_${convId}` : 'wok_analytics_global';
}

export function trackAnalyticsEvent(convId, extra = {}) {
  try {
    const key = getAnalyticsKey(convId);
    const events = JSON.parse(localStorage.getItem(key) || '[]');
    events.push({
      ts: Date.now(),
      sid: localStorage.getItem('wok_sid') || (() => {
        const id = `s_${Math.random().toString(36).slice(2)}`;
        localStorage.setItem('wok_sid', id);
        return id;
      })(),
    ...
    localStorage.setItem(key, JSON.stringify(events.slice(-10000)));
  } catch {}
}

export default function AnalyticsPanel({ convId }) {
  ...
  useEffect(() => {
    ...
    setData({
      ...
      devices: toRows(devices).map(r => ({ ...r, value: pageviews ? `${Math.round(r.value/pageviews*100)}%` : '0%' })),
    });
    setLoading(false);
  }, [range, convId]);
