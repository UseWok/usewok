...
function MetricRow({ label, color, value, max }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      <span style={{ width: 76, fontSize: 12.5, fontWeight: 600, color, flexShrink: 0 }}>{label}</span>
    ...
    <DashCard title="Évolution" dot={ORANGE}>
      ...
      <div style={{ display: 'flex', gap: 28, marginBottom: 20 }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 12, justifyContent: 'center', minWidth: 0 }}>
          <MetricRow label="Narratif" color={ORANGE_DEEP} value={b.narrative || 0} max={maxBar} />
          <MetricRow label="Autorité" color={INK} value={b.authority || 0} max={maxBar} />
          <MetricRow label="Référence" color={AMBER_DEEP} value={b.referral || 0} max={maxBar} />
        </div>
      </div>
      <div style={{ fontSize: 11.5, color: INK3, paddingTop: 14, borderTop: '1px solid rgba(21,19,15,0.09)', marginBottom: 18 }}>
        {b.brand_pct ?? 40}% Marque · {b.website_pct ?? 30}% Site web · {b.earned_pct ?? 30}% Externe
      </div>
