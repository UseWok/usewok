...
import { Loader2, LayoutTemplate, Settings, Copy, AlertTriangle, Trash2, LayoutDashboard, Share2, ExternalLink, Sparkles, Code2, FileCode, CheckCircle2, Pencil, Maximize2, Minimize2, MoreHorizontal, Download } from 'lucide-react';
...
`;
  ...
  return (
    <div className="w-full h-full relative font-sans overflow-hidden">
      {hasComponent ? (
        viewMode === 'preview' ? (
          <div className={`${isFullscreen ? 'fixed inset-0 z-[9999]' : 'w-full h-full'} relative bg-white overflow-hidden`}>
            {/* ── Preview Toolbar — top left ── */}
            <div style={{ position: 'absolute', top: 10, left: 10, zIndex: 25, display: 'flex', alignItems: 'center', gap: 6 }} ref={optionsMenuRef}>
              {/* Fullscreen toggle */}
              <button
                onClick={() => setIsFullscreen(v => !v)}
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 28, height: 28, borderRadius: 7, border: 'none', cursor: 'pointer', background: 'rgba(20,20,20,0.8)', color: '#ccc', backdropFilter: 'blur(8px)', transition: 'color 150ms' }}
              ...
              <div style={{ position: 'relative' }}>
                <button
                  ...
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 28, height: 28, borderRadius: 7, border: 'none', cursor: 'pointer', background: showOptionsMenu ? 'rgba(249,87,56,0.15)' : 'rgba(20,20,20,0.8)', color: showOptionsMenu ? '#F95738' : '#ccc', backdropFilter: 'blur(8px)', transition: 'all 150ms' }}
                  title="Options"
                >
                ...
                <AnimatePresence>
                  {showOptionsMenu && (
                    <motion.div
                      ...
                      style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 0, background: '#141414', border: '1px solid #2A2A2A', borderRadius: 9, boxShadow: '0 8px 24px rgba(0,0,0,0.5)', zIndex: 100, minWidth: 160, overflow: 'hidden' }}
                    >
                      <button
                        onClick={handleExportZip}
                        style={{ display: 'flex', alignItems: 'center', gap: 9, width: '100%', padding: '10px 14px', background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, color: '#ccc', fontFamily: 'Inter, sans-serif', textAlign: 'left' }}
              ...
              {isFullscreen && (
                <button
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 10px', borderRadius: 7, border: 'none', cursor: 'pointer', background: 'rgba(20,20,20,0.8)', color: '#ccc', fontSize: 11, fontFamily: 'Inter, sans-serif', backdropFilter: 'blur(8px)' }}
                >
                  <Minimize2 style={{ width: 11, height: 11 }} /> Exit fullscreen
                </button>
              )}
            </div>
