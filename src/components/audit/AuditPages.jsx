  ...
  return (
    <div style={{ fontFamily: F }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          <p style={{ fontSize: 22, fontWeight: 800, color: INK, margin: '0 0 4px', letterSpacing: '-0.03em' }}>Pages crawled</p>
          <p style={{ fontSize: 13, color: INK3, margin: 0 }}>{pages.length} URL{pages.length !== 1 ? 's' : ''} analyzed</p>
        </div>
      ...
      {pages.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '60px 20px', color: INK3, fontSize: 13 }}>No analyzed pages available.</div>
      ) : (
        <div style={{ background: WHITE, border: `1px solid ${BORDER}`, borderRadius: 12, overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '32px 36px 1fr 90px 90px 90px 70px', background: SURFACE, borderBottom: `1px solid ${BORDER}`, padding: '9px 14px', alignItems: 'center', gap: 0 }}>
            ...
            <span style={{ fontSize: 10, fontWeight: 600, color: INK3, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Status</span>
            <span style={{ fontSize: 10, fontWeight: 600, color: INK3, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Frequency</span>
            <span style={{ fontSize: 10, fontWeight: 600, color: INK3, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Depth</span>
            <span style={{ fontSize: 10, fontWeight: 600, color: INK3, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Issues</span>
          </div>
            ...
            {pages.map((page, i) => (
              <div key={i}
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  {!page.indexable && <span style={{ fontSize: 10, color: INK3 }}>Not indexable</span>}
                </div>
                ...
                <span style={{ fontSize: 11, color: INK2 }}>{page.crawl_depth ? `${page.crawl_depth} click${page.crawl_depth > 1 ? 's' : ''}` : '–'}</span>
