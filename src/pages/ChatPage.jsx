...
export default function ChatPage() {
  ...
  const handleCreateWorkspace = () => {
    ...
    if (workspaces.length >= 4) { toast.error("Limit of 4 workspaces reached."); return; }
  ...
  return (
    <div className="flex font-sans h-screen w-full bg-white overflow-hidden antialiased">
      ...
      <ProModal open={showWorkspaceModal} onClose={() => setShowWorkspaceModal(false)} title="Create a workspace" subtitle="Start collaborating with your workspace members" actionText="Create workspace" onAction={handleCreateWorkspace}>
        ...
        <input type="text" value={newWorkspaceName} onChange={(e) => setNewWorkspaceName(e.target.value)} placeholder="Choose a name..." className="w-full border border-[#E5E5E5] rounded-md px-3 py-2 text-[13px] focus:outline-none focus:border-[#0080ff]" autoFocus />
      </ProModal>

      <ProModal open={showCodeModal} onClose={() => setShowCodeModal(false)} title="Redeem Code" subtitle="Enter your promotional or access code below." actionText="Apply Code" onAction={() => { toast.success("Code applied"); setShowCodeModal(false); }}>
      ...
      {isSidebarOpen && (
        <aside className="w-[260px] flex-shrink-0 h-full border-r border-[#E5E5E5] flex flex-col z-40" style={{ background: 'linear-gradient(180deg, #F8FAFC 0%, #EAEFF8 100%)' }}>
          ...
          <div className="px-4 py-3 border-b border-black/5"><button onClick={() => navigate('/')} className="flex items-center justify-center gap-2 w-full py-2.5 bg-[#0080ff] text-white rounded-md text-[13px] font-bold hover:bg-[#0066cc] shadow-sm"><Plus className="w-4 h-4" /> New chat</button></div>

          <div className="px-4 space-y-0.5 mt-3">
            {navItems.map((item) => (
              <button key={item.label} onClick={() => navigate(item.path)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-md text-[13px] font-medium ${item.active ? 'bg-white shadow-sm border border-[#E5E5E5] text-gray-900 font-bold' : 'text-gray-600 hover:bg-white/50 border border-transparent'}`}>
          ...
          <div className="flex-1 overflow-y-auto px-4 mt-6">
             <div className="text-[11px] font-bold text-gray-400 mb-3 px-1 tracking-wider uppercase">Recents</div>
             <ul className="space-y-0.5">
                {discussions?.map((d, idx) => (
                  <li key={d.id} draggable onDragStart={() => setDraggedItemIdx(idx)} onDragOver={(e) => { e.preventDefault(); setDragOverIdx(idx); }} onDrop={() => handleDrop(idx)} onClick={() => navigate(`/chat?conversationId=${d.id}`)} className={`relative flex items-center justify-between px-3 py-2.5 rounded-md cursor-pointer group ${conversationId === d.id ? 'bg-white shadow-sm border border-[#E5E5E5]' : 'border border-transparent hover:bg-white/50'}`}>
                    ...
                    <div className="hidden group-hover:flex items-center gap-1.5"><button onClick={(e) => startEditing(e, d)}><Edit2 className="w-3.5 h-3.5" /></button><button onClick={(e) => deleteDiscussion(e, d.id)}><Trash2 className="w-3.5 h-3.5" /></button></div>
                  </li>
                ))}
             </ul>
          </div>
          ...
          <div className="p-4 border-t border-black/5 relative" ref={profileMenuRef}>
            {isProfileMenuOpen && (
              <div className="absolute bottom-[calc(100%+12px)] left-4 w-[240px] bg-white border border-[#E5E5E5] rounded-xl shadow-2xl py-1.5 z-50 font-sans p-1.5">
                ...
                {/* 95% MODAL TRIGGERS */}
                <button onClick={() => { setIsProfileMenuOpen(false); setIframeModal({open:true, url:'/settings'}); }} className="w-full text-left px-3 py-2 text-[13px] text-[#707070] hover:bg-gray-50 flex items-center gap-2.5 rounded-lg transition-colors"><Settings className="w-4 h-4" /> Settings</button>
                <button onClick={() => { setIsProfileMenuOpen(false); setIframeModal({open:true, url:'/support'}); }} className="w-full text-left px-3 py-2 text-[13px] text-[#707070] hover:bg-gray-50 flex items-center gap-2.5 rounded-lg transition-colors"><LifeBuoy className="w-4 h-4" /> Support</button>
                <div className="h-px bg-[#E5E5E5] my-1 mx-2"></div>
                <button onClick={() => { setIsProfileMenuOpen(false); navigate('/pricing'); }} className="w-full text-left px-3 py-2 text-[13px] text-[#333333] font-semibold hover:bg-gray-50 flex items-center gap-2.5 group rounded-lg"><ArrowUpCircle className="w-4 h-4 text-[#0080ff]" /> Upgrade plan</button>
            ...
            <button onClick={() => setIsProfileMenuOpen(!isProfileMenuOpen)} className="flex items-center gap-3 p-2 rounded-xl hover:bg-white/50 border border-transparent hover:border-[#E5E5E5] transition-colors w-full text-left">
              <div className="w-9 h-9 rounded-lg flex items-center justify-center text-white text-[13px] font-bold shadow-sm" style={{ backgroundColor: getUserColor(user) }}>{(user?.full_name || 'U').charAt(0).toUpperCase()}</div>
              <div className="flex-1 min-w-0"><p className="text-[13px] font-bold text-[#333333] truncate">{user?.full_name || 'User'}</p><p className="text-[11px] text-[#707070] font-medium">Free Plan</p></div>
      ...
      <div className="flex-1 flex overflow-hidden bg-white relative">
        <div className="absolute top-4 left-4 z-20"><button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 text-gray-400 hover:text-gray-800 hover:bg-gray-50 transition-colors rounded-md bg-white border border-[#E5E5E5] shadow-sm">{isSidebarOpen ? <PanelLeftClose className="w-4 h-4" /> : <PanelLeft className="w-4 h-4" />}</button></div>
        <div className="flex flex-1 overflow-hidden w-full h-full">
          <div className={`flex flex-col bg-white overflow-hidden transition-all duration-300 ${hasStarted ? 'w-1/4 min-w-[320px] max-w-[400px] border-r border-[#E5E5E5]' : 'w-full h-full justify-center max-w-3xl mx-auto'}`}>
          ...
          {hasStarted && (
            <div className="flex-1 bg-white p-3 overflow-hidden flex flex-col">
              <div className={`w-full h-full flex flex-col overflow-hidden transition-all duration-300 border rounded-md border-[#E5E5E5] bg-white shadow-sm`}>
