      ...
      <body>
        ...
        <script>
          ...
          window.addEventListener('message', function(e) {
            ...
            try {
              // Strip import statements — re-added as proper ESM in the blob below
              var codeNoImports = code.replace(/^import\s[\s\S]*?from\s+['"][^'"]+['"]\s*;?\s*$/gm, '');

              var transformed = Babel.transform(codeNoImports, {
                presets: [
                  ['env', { targets: { chrome: 90 }, modules: false }],
                  'react',
                ],
              }).code;

              var moduleCode = [
                'import React, { useState, useEffect, useRef, useCallback, useMemo, useContext, createContext, forwardRef, memo, Fragment } from "react";',
                'import { createRoot } from "react-dom/client";',
                'import { motion, AnimatePresence, useMotionValue, useTransform, useSpring, useAnimate } from "framer-motion";',
                'import { BarChart, Bar, LineChart, Line, AreaChart, Area, PieChart, Pie, Cell, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, RadarChart, Radar, PolarGrid, PolarAngleAxis, ScatterChart, Scatter } from "recharts";',
                'import { Home, Search, Settings, User, Bell, Menu, X, ChevronDown, ChevronUp, ChevronLeft, ChevronRight, ArrowRight, ArrowLeft, Check, Plus, Minus, Edit, Trash2 as Trash, Save, Download, Upload, Share, Eye, EyeOff, Lock, Unlock, Star, Heart, Bookmark, Flag, Tag, Filter, Grid, List, Calendar, Clock, Mail, Phone, Globe, Map, Camera, Image, Video, Music, File, Folder, Code, Terminal, Database, Server, Cloud, Wifi, Battery, Sun, Moon, Zap, Shield, Award, Gift, Package, ShoppingCart, CreditCard, DollarSign, TrendingUp, TrendingDown, BarChart2, Activity, Target, Layers, Box, Circle, Square, Triangle, Hexagon, Maximize, Minimize, RefreshCw, RotateCw, Shuffle, AlertCircle, AlertTriangle, Info, HelpCircle, MessageCircle, MessageSquare, Send, Inbox, Archive, Paperclip, Link, ExternalLink, Copy, Scissors, Clipboard, Printer, Mic, MicOff, Volume2, VolumeX, Play, Pause, SkipBack, SkipForward, Repeat, Loader, Feather, Coffee, Smile, Meh, Frown, ThumbsUp, ThumbsDown, Users, UserPlus, UserMinus, LogIn, LogOut, Key, Hash, AtSign, Percent, Slash } from "lucide-react";',
                'window.React = React;',
                'window.useState = useState; window.useEffect = useEffect; window.useRef = useRef; window.useCallback = useCallback; window.useMemo = useMemo; window.useContext = useContext; window.createContext = createContext; window.forwardRef = forwardRef; window.memo = memo; window.Fragment = Fragment;',
                transformed,
                '',
