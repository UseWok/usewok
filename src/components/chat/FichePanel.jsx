  ...
  return (
    <div className="flex-1 overflow-y-auto p-8 md:p-12 max-w-4xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        {/* Badge Control (Hidden Visually from HTML but left in logic to avoid breaking appSettings) */}
        <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm opacity-50 pointer-events-none">
          <h3 className="text-[15px] font-bold text-slate-900 mb-1">White Label</h3>
          <p className="text-[13px] text-slate-500 mb-5">Watermark is disabled globally for this tier.</p>
          <div className="flex items-center justify-between mt-2 p-3 bg-slate-50 rounded-lg border border-slate-100">
            ...
            <button
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors bg-slate-300`}
            >
...
export default function FichePanel({
  ...
  useEffect(() => {
    ...
    if (content) {
      ...
      // 1. EXTRACTION PROTOCOL: Hunts strictly for the JSX code block. 
      // This kills the "Blank iframe" bug caused by conversational text sneaking into Babel.
      ...
      setCompiledCode({ html, css, js, rawComponent: extractedCode });
