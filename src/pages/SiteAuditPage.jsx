...
export default function SiteAuditPage() {
  ...
  const fmtDate = d => new Date(d).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }).replace(':', ':');
    ...
    <div style={{ minHeight: '100vh', background: '#F7F5F0', fontFamily: F }}>
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: '24px 24px 80px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: INK, margin: 0, letterSpacing: '-0.03em' }}>Audit Website</h1>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {nextDate && (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '7px 14px', border: '1px solid rgba(16,185,129,0.35)', borderRadius: 20, background: 'rgba(16,185,129,0.06)', fontSize: 12, color: '#0B815A' }}>
                <RefreshCw size={11} />
                Prochain audit automatique <strong>{nextDate.toLocaleDateString('fr-FR', { day: '2-digit', month: 'long', year: 'numeric' })}</strong>
              </span>
            )}
        ...
        <div style={{ background: '#fff', border: `1px solid ${BORDER}`, borderRadius: 14, overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 0.9fr 0.9fr 0.9fr', padding: '13px 20px', borderBottom: `1px solid ${BORDER}` }}>
            {['Date', 'Pages analysées', 'Statut', 'Agents', 'Score Website'].map(h => (
          ...
          {!loading && audits.length === 0 && !running && (
            <p style={{ fontSize: 13, color: INK3, textAlign: 'center', padding: '32px 0' }}>Aucun audit pour le moment. Lancez votre premier audit du site.</p>
          )}
          {running && (
