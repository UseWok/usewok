...
import { RefreshCw, Palette, ExternalLink, ArrowLeft, Link as LinkIcon, Mail } from 'lucide-react';
...
export default function WorkspaceHeader({ onReload, appearance, setAppearance, onAskAI, convId }) {
  ...
  const [publishView, setPublishView] = useState('main'); // 'main' | 'share'
      ...
      <header className={`flex items-center justify-between px-4 h-[48px] flex-shrink-0 z-30 font-sans w-full rounded-t-md ${appearance?.theme !== 'classic' ? 'bg-transparent border-b border-black/5' : 'bg-[#F4F4F4] border-b border-[#E5E5E5]'}`}>
        ...
        <div className="flex justify-center w-1/2 gap-2 relative">
          ...
          <div className="relative" ref={appRef}>
            ...
            {showAppearance && (
              <div className="absolute top-[calc(100%+6px)] left-1/2 -translate-x-1/2 w-[340px] bg-white border border-[#E5E5E5] rounded-lg shadow-2xl z-50 p-1 text-left font-sans text-[#333333]">
          ...
          <div className="relative" ref={modeRef}>
            <button onClick={() => setShowMode(!showMode)} className={`px-3.5 py-1.5 rounded-md text-[12px] font-semibold flex items-center gap-1.5 shadow-sm transition-colors border ${isDark ? 'bg-white/10 border-white/20 text-white hover:bg-white/20' : 'bg-white border-[#E5E5E5] text-[#333333] hover:bg-gray-50'}`}>
              {isDeepWork ? 'Deep Work' : 'Automatic'}
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="6 9 12 15 18 9"/></svg>
            </button>

            {showMode && (
              <div className="absolute top-[calc(100%+6px)] w-[300px] bg-white border border-[#E5E5E5] rounded-lg shadow-2xl z-50 p-1 text-left text-[#333333]">
                <div className="p-3 hover:bg-gray-50 cursor-pointer rounded-md transition-colors" onClick={() => { setIsDeepWork(false); setShowMode(false); }}>
                  <div className="text-[13px] font-bold flex items-center gap-2">Automatic {(!isDeepWork) && <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#0080ff" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>}</div>
                  <div className="text-[11.5px] text-[#707070] mt-1 leading-snug">Wok intelligently selects the best logic engine based on context.</div>
                </div>
                <div className="h-px bg-[#E5E5E5] my-1 mx-2"></div>
                <div className="p-3 flex items-center justify-between rounded-md">
                  <div className="flex-1 pr-3">
                    <div className="text-[13px] font-bold">Deep Work</div>
        ...
        <div className="flex justify-end items-center gap-2 w-1/4 relative" ref={publishRef}>
          ...
          <button onClick={() => setShowPublish(!showPublish)} className="px-3.5 py-1.5 bg-[#0080ff] text-white text-[12px] font-bold rounded-md hover:bg-[#0066cc] shadow-sm">
            Publish
          </button>
                  ...
                  <div className="px-3 pb-2 pt-1">
                    {indexGoogle && (
                      <div className="bg-[#F9F8F6] border border-[#E5E5E5] p-2.5 rounded-md flex items-start gap-2 mb-3 mt-2">
                        <div className="w-1.5 h-1.5 rounded-full bg-[#DDFF00] mt-1 flex-shrink-0"></div>
